...
Speed & size options:
    --sample-rate 0.1   Keep only this fraction of vessels (default 1.0 = all).
                        Sampled deterministically by MMSI so the same ship is
                        always included/excluded across every file.  Keeps ALL
                        pings for selected vessels → complete routes.
                        Combine with --min-interval 0 for dense trajectories.
    --min-interval 60   Keep at most one ping per MMSI per N seconds (default 0
                        when --sample-rate < 1, else 60). Useful for reducing
                        ping density when keeping all vessels.
    --workers 4         Number of parallel file-parsing processes (default: half
                        your CPU cores). Each worker parses one file at a time;
                        the main process handles all SQLite writes.
