...
class SimEngine:
    ...
    def __init__(self, db_path: str, checkpoint_path: str, device: str = "cpu"):
        ...
        # Online z-score calibration. The checkpoint's variance head is not
        # guaranteed to be calibrated (the current one over-estimates sigma,
        # so raw z-scores max out around 0.1 and no UI threshold in the 1–5σ
        # range could ever fire). We rescale raw z by the fleet's recent
        # median so the calibrated score behaves like a unit-normal deviate:
        # threshold 3σ then means "3× the typical current deviation".
        self._z_recent: list = []       # rolling raw scores (bounded)
        self._z_scale        = None     # raw-z median / 0.6745
    ...
    def _load_model(self):
        with self._model_lock:
            if self._model is None:
                print("[SimEngine] loading model ...", flush=True)
                self._model = load_model(self.checkpoint_path, self.device)
    ...
    def _prepare_db(self):
        ...
        try:
            ...
            # Per-cell normal dwell: the 90th-percentile duration of *contiguous*
            # stationary visits in each cell. Loitering is then judged relative to
            # what is normal for that location — a port where ships wait 3 days has
            # a large threshold; open water has none (uses LOITER_OPEN_SEC).
            self._build_dwell_table(conn, cur)
            cur.execute(f"SELECT ilat, ilon, thresh_sec FROM {_DWELL_TABLE}")
            self._cell_dwell = {(r[0], r[1]): r[2] for r in cur.fetchall()}
            print(f"[SimEngine] {len(self._anchor_cells)} anchorage cells, "
                  f"{len(self._cell_dwell)} dwell cells; "
    ...
    def _build_dwell_table(self, conn, cur):
        """Learn per-cell normal dwell from contiguous stationary visits.

        A 'visit' is a maximal run of low-SOG pings with < DWELL_RUN_GAP_SEC
        between consecutive pings; its dwell is (last - first) timestamp. The
        per-cell threshold is the DWELL_PCTL percentile of visit dwells, so
        loitering is measured against how long ships *normally* stay there.
        Built once (~1–2 min on a large DB) and cached in _DWELL_TABLE."""
        cur.execute(
        ...
        if cur.fetchone() is not None:
            ...
            if cur.fetchone()[0] > 0:
                return
        print("[SimEngine] learning per-cell dwell norms (one-time, ~1–2 min) ...",
              flush=True)
        rows = cur.execute(
            "WITH s AS ("
            "  SELECT MMSI,"
        ...
        by_cell: dict = {}
        for ilat, ilon, dwell_sec, _ in rows:
        ...
        cur.execute(f"CREATE TABLE {_DWELL_TABLE} "
                    "(ilat INT, ilon INT, thresh_sec REAL, n INT)")
        cur.executemany(
            f"INSERT INTO {_DWELL_TABLE} VALUES (?,?,?,?)",
            [(ilat, ilon, float(np.percentile(v, DWELL_PCTL)), len(v))
    ...
    def _calibrate_z(self, raw: float) -> float:
        """Rescale a raw z-score by the fleet's recent 90th-percentile
    ...
    def _handle_ping(self, row, ping_ts, frame_events):
        ...
        with self._vessels_lock:
            ...
            vs.last_ts  = ping_ts
            try:
                vs.ship_type_group = cfg.ship_type_groups.get(
                    int(float(row["SHIP_TYPE"] or 0)), 7)
            except (ValueError, TypeError):
                vs.ship_type_group = 7
            ...
            lat_norm = (vs.last_lat - LAT_LO) / LAT_RNG
            lon_norm = (vs.last_lon - LON_LO) / LON_RNG
            best_score = None
            best_step  = None
            per_pred_scores = []
            ...
            for pr in vs.preds:
                ...
                if step < SEQ_DEC:
                    z_lat = abs(lat_norm - float(pr.mu[step, 0])) / (float(pr.sigma[step, 0]) + 1e-8)
                    z_lon = abs(lon_norm - float(pr.mu[step, 1])) / (float(pr.sigma[step, 1]) + 1e-8)
                    raw   = float((z_lat + z_lon) / 2.0)
    ...
    def _run_detectors(self, vs, prev_lat, prev_lon, prev_ts, ping_ts):
        ...
        elif not math.isnan(ping_ts):
            dwell = ping_ts - vs.loiter_since
            norm  = self._cell_dwell.get(self._cell(vs.loiter_lat, vs.loiter_lon))
            thresh = max(LOITER_MIN_SEC, norm * LOITER_MARGIN) if norm else LOITER_OPEN_SEC
            if dwell >= thresh and self._cooldown_ok(vs, "loitering"):
                hrs  = dwell / 3600.0
                note = (f"normal here ~{norm/3600:.0f} h" if norm
                        else "open water — no normal stop")
    ...
    def _batch_ar_predict(self, enc_arrays):
        ...
        for _ in range(SEQ_DEC):
            mu_raw, log_var = self._model(src, dec_input)
