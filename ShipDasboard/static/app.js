...
fetch("/api/ships")
  ...
  .then(data => {
    ...
    // Replay anomalies are classified by the same σ threshold as the live sim.
    if (REPLAY_THRESH != null) {
      const t = document.getElementById("ap-thresh-note");
      if (t) t.textContent = `flagged at ≥ ${REPLAY_THRESH.toFixed(1)}σ (same as Live Sim)`;
    }
...
function buildAnomalyList() {
  ...
  const top = ships.filter(s => s.level === "severe" || s.level === "moderate").slice(0, 8);
  container.innerHTML = top.map(s => {
    const z = s.z_peak != null ? `${s.z_peak.toFixed(1)}σ` : `${s.ade_km.toFixed(1)} km`;
    return `
    <div class="anomaly-entry ${s.level}" onclick="jumpToShip(${s.id})">
      <div class="ship-label">${s.type_name} &mdash; ${z}</div>
      <div class="ship-reason">${s.reason.split(";")[0]}</div>
    </div>`; }).join("");
}

// ── Vessel icon: AIS triangle for known types, circle for unknown (type 0) ───
function _vesselIcon(color, cogDeg, level, isSelected, shipType) {
  const sizes = { severe: 14, moderate: 13, mild: 12, none: 11 };
  const s  = isSelected ? 15 : (sizes[level] || 11);
  const sc = isSelected          ? "#fff"     :
             level === "severe"   ? "#ef4444" :
             level === "moderate" ? "#f97316" :
             level === "mild"     ? "#f59e0b" : "rgba(30,30,30,0.35)";
  const sw = (isSelected || level !== "none") ? 1.5 : 0.6;
...
function createMarkers() {
  ...
  markers = ships.map(ship => {
    ...
    const icon = _vesselIcon(col, cog, ship.level, false, ship.type);
    ...
    // Ground-truth future: drawn progressively so prediction-vs-reality
    // deviation is visible on the map.
    ...
    // tick passes its issued_step (added lazily in updateTick).
...
function makeTooltip(ship) {
  ...
  return `<b>${ship.type_name}</b><br>SOG: ${sog} kn &nbsp; ADE: ${ship.ade_km.toFixed(1)} km<br>${levelBadge}<br>
...
function updateTick(tick) {
  ...
  if (!simMode) {
    phase.textContent = atEnd ? "⬤ COMPLETE" : inDecoder ? "⬤ PREDICTING" : "⬤ HISTORY";
  ...
  ships.forEach((ship, i) => {
    const m = markers[i];
    if (!m) return;
    const vis = activeTypes.has(ship.type) && activeLevels.has(ship.level);
    if (vis !== m._vis) {
      m._vis = vis;
    ...
    m.predLines.forEach(pl => {
      ...
      if (visible && !pl.shown) {
        ...
        pl.line.setLatLngs([origin, ...pl.pred.coords]);
        pl.line.addTo(map);
        pl.shown = true;
        changed = true;
...
function updateStats() {
  ...
  const anomalous = ships.filter(s => s.level !== "none").length;
...
function selectReplayShip(id) {
  ...
  m.marker.setIcon(_vesselIcon(TYPE_COLOR[ship.type] || "#64748b",
                               _tickCog(ship, currentTick), ship.level, true, ship.type));
...
function deselectReplayShip() {
  ...
  if (ship && m) {
    m.marker.setIcon(_vesselIcon(TYPE_COLOR[ship.type] || "#64748b",
                                 _tickCog(ship, currentTick), ship.level, false, ship.type));
...
function refreshReplaySelPanel(id) {
  ...
  document.getElementById("sel-type").textContent =
    `${ship.type_name} · replay · ${ship.level}`;
  ...
  if (!newest) {
    ...
    html += `<div class="sel-pred-title">Prediction vs ground truth</div>`;
    revealed.slice(-4).reverse().forEach(p => {
      const col = p.ade_km > 3 ? "#ef4444" : p.ade_km > 1 ? "#f97316" : "#22c55e";
      html += `<div class="sel-cmp-row">
        <span class="sel-cmp-age">#${p.pred_id + 1} @ step ${p.issued_step}</span>
        <span class="sel-cmp-drift" style="color:${col}">${p.ade_km.toFixed(2)} km</span></div>`;
    });
    ...
    if (revealed.length > 1) {
      ...
      for (let i = revealed.length - 2; i >= Math.max(0, revealed.length - 4); i--) {
        ...
        for (let st = newest.issued_step;
             st < older.issued_step + older.coords.length &&
             st < newest.issued_step + newest.coords.length; st++) {
          sum += _haversineKm(older.coords[st - older.issued_step],
                              newest.coords[st - newest.issued_step]);
          k++;
        }
...
function _enterSimMode() {
  ...
  const phase = document.getElementById("phase-badge");
  phase.textContent = "⬤ LIVE"; phase.className = "sim";
...
function applySimStatus(s) {
  ...
  const colors = { running: "#22c55e", paused: "#f59e0b", seeking: "#1f6f8b",
...
function onSimPing(ev) {
  ...
  const stateText = holding
    ...
    : n_pings < SEQ_ENC
      ? `<br><small style="color:#1f6f8b">Warming up… ${n_pings}/${SEQ_ENC} pings</small>`
      : v.inferPending
        ? `<br><small style="color:#1f6f8b">⚡ predicting…</small>` : "";
...
const ANOM_KINDS = {
  speed_jump:      { col: "#FFE4E1", label: "Position jump" },
  on_land:         { col: "#A8E6CF", label: "On land" },
  impossible_sog:  { col: "#D0F0C0", label: "Impossible speed" },
  loitering:       { col: "#F5FFFA", label: "Loitering" },
  model_deviation: { col: "#004D00", label: "Off predicted path" },
};
...
const _feedByMmsi = new Map();   // mmsi -> { el, kinds: {kind: n} }
...
function onSimAnomaly(ev) {
  ...
  if (!entry) {
    entry = { el: document.createElement("div"), kinds: {} };
    entry.el.className = "feed-entry";
    entry.el.dataset.mmsi = mmsi;
    entry.el.onclick = () => {
      const v = simVessels.get(mmsi);
      if (v) map.setView(v.marker.getLatLng(), 11, { animate: true });
    };
  ...
  const chips = Object.entries(entry.kinds).map(([k, n]) => {
    ...
    return `<span class="feed-chip" style="background:${m.col};color:${m.txt}"
              title="${m.label}">${m.label}${n > 1 ? " ×" + n : ""}</span>`;
  }).join("");
  ...
  entry.el.innerHTML =
    `<span class="feed-total-badge" style="background:${col};color:${txt}" title="${total} alert${total > 1 ? "s" : ""}">×${total}</span>` +
...
function refreshSelPanel(mmsi) {
  ...
  const pendingHtml = v.inferPending
    ? `<div class="sel-watch" style="color:#a7d7e0">⚡ Inference running…</div>` : "";
