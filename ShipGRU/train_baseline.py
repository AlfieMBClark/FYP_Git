...
Usage:
    python train_baseline.py               # full 30 epochs
    python train_baseline.py --smoke-test  # 1 epoch to verify pipeline
    python train_baseline.py --epochs 10   # custom epoch count
"""
...
GRU_DROPOUT   = 0.2
GRU_EPOCHS    = 50
CKPT_PATH     = os.path.join(_HERE, "checkpoints", "baseline_model.pt")
LOG_PATH      = os.path.join(_HERE, "logs", "baseline_training.log")


# ── Land mask ──────────────────────────────────────────────────────────────────
# Differentiable land-avoidance penalty, mirroring ShipTransformer/train.py.
# The predicted absolute positions are sampled against an OSM-derived water raster;
# predictions that fall on land incur a penalty proportional to how far inland.
    ...
    with torch.set_grad_enabled(train):
        for batch_idx, (src, tgt) in enumerate(loader):
            ...
            if train:
                ...
                # model.predict() is not used because it feeds mu directly back as an
                # absolute position; with predict_deltas the output is dLAT/dLON so
                # we must accumulate before constructing the next decoder input.
