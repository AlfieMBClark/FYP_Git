"""
predict_tcn.py — inference and anomaly scoring for ShipTCNModel.
Mirrors predict.py / predict_baseline.py; adds --compare for side-by-side Transformer vs TCN.

Usage
-----
python predict_tcn.py --random --plot
python predict_tcn.py --mmsi 219123456 --window 0
python predict_tcn.py --random --count 10
python predict_tcn.py --mmsi 219123456 --all-windows
python predict_tcn.py --compare                     # same window, both models
python predict_tcn.py --compare --random --plot     # random vessel, both models, map
"""
...
def load_tcn_model(checkpoint_path, device):
    ...
    model = ShipTCNModel(
        ...
        dropout      = 0.0,
        seq_len_enc  = c.get("seq_len_enc",  SEQ_ENC),
...
@torch.no_grad()
def predict_tcn(model: ShipTCNModel, src_np: np.ndarray, device) -> tuple[np.ndarray, np.ndarray]:
    """Autoregressive TCN inference; reuses last observed DT for all future steps."""
...
def print_compare_table(mmsi, w_idx, dec_actual_norm,
                        ...
                        tcn_mu, tcn_sigma,
                        threshold):
    """Side-by-side Transformer vs TCN comparison table."""
    tf_pred  = denormalise_dec(tf_mu)
    tcn_pred = denormalise_dec(tcn_mu)
    actual   = denormalise(dec_actual_norm)

    tf_z     = _z_score(dec_actual_norm, tf_mu,  tf_sigma)
    tcn_z    = _z_score(dec_actual_norm, tcn_mu, tcn_sigma)

    tf_dists  = [_haversine_km(actual[i, 0], actual[i, 1], tf_pred[i,  0], tf_pred[i,  1]) for i in range(SEQ_DEC)]
    tcn_dists = [_haversine_km(actual[i, 0], actual[i, 1], tcn_pred[i, 0], tcn_pred[i, 1]) for i in range(SEQ_DEC)]

    tf_ade  = float(np.mean(tf_dists))
    tcn_ade = float(np.mean(tcn_dists))
    tf_fde  = tf_dists[-1]
    tcn_fde = tcn_dists[-1]
    tf_mz   = float(tf_z.mean())
    tcn_mz  = float(tcn_z.mean())

    tf_verdict  = "SUSPICIOUS" if tf_mz  > threshold else "NORMAL"
    tcn_verdict = "SUSPICIOUS" if tcn_mz > threshold else "NORMAL"

    sep = "─" * 49
    print(f"\nMMSI: {mmsi}  Window: {w_idx}")
    print(sep)
    print(f"{'':20}{'Transformer':>14}      {'TCN':>14}")
    print(sep)
    print(f"{'Mean ADE':<20}{tf_ade:>11.2f} km   {tcn_ade:>11.2f} km")
    print(f"{'Final FDE':<20}{tf_fde:>11.2f} km   {tcn_fde:>11.2f} km")
    print(f"{'Mean Z-score':<20}{tf_mz:>14.2f}   {tcn_mz:>14.2f}")
    print(f"{'Verdict':<20}{tf_verdict:>14}   {tcn_verdict:>14}")
    print(sep)
    for i in range(SEQ_DEC):
        print(f"Step {i+1:>2}             {tf_dists[i]:>8.1f} km   {tcn_dists[i]:>8.1f} km")
    print(sep)


# ── Map / plot helpers ────────────────────────────────────────────────────────

def save_compare_map(enc_raw, dec_actual_norm, tf_mu, tcn_mu, mmsi, out_path):
    """Folium map: red=Transformer, orange=TCN, green=actual."""
    try:
        import folium
    except ImportError:
    ...
    tcn_pred_raw   = denormalise_dec(tcn_mu)
    ...
    # TCN prediction
    tcn_route = [coords(enc_raw[-1:])[0]] + coords(tcn_pred_raw)
    folium.PolyLine(tcn_route, color="orange", weight=2, dash_array="8",
                    tooltip="TCN").add_to(m)
    for i, row in enumerate(tcn_pred_raw):
        folium.CircleMarker(
            [float(row[0]), float(row[1])], radius=6, color="orange",
            fill=True, fill_color="white", fill_opacity=0.9,
            tooltip=f"TCN step {i+1}  Lat {row[0]:.4f}  Lon {row[1]:.4f}",
        ).add_to(m)
    ...
    legend_html = (
        ...
        '<div style="color:orange">- - TCN predicted</div>'
        '</div>'
    )
...
def _add_vessel_to_map(m, full_track_raw, enc_actual_raw, dec_actual_norm, mu_pred, mmsi, color):
    ...
    folium.PolyLine(
        ...
        tooltip=f"MMSI {mmsi} — TCN predicted",
    ).add_to(m)
...
def save_folium_map(vessels, out_path):
    ...
    legend_html = (
        ...
        'background:white;padding:10px 14px;border-radius:6px;'
        'box-shadow:0 1px 5px rgba(0,0,0,.4);font-family:sans-serif;font-size:13px">'
        f'<b>TCN — Vessels ({len(vessels)})</b><br><br>'
        f'{legend_items}'
...
def _process_vessel(mmsi, args, tcn_model):
    ...
    tcn_mu, tcn_sigma = predict_tcn(tcn_model, enc, args.device)
    enc_raw        = denormalise(enc)
    full_track_raw = denormalise(track_norm)

    print_window_result(dec, tcn_mu, tcn_sigma, args.anomaly_threshold)

    return (full_track_raw, enc_raw, dec, tcn_mu, mmsi)


# ── Main ──────────────────────────────────────────────────────────────────────

def main():
    parser = argparse.ArgumentParser(description="Predict with TCN or compare with Transformer.")
    ...
    parser.add_argument("--compare",            action="store_true",
                        help="Run both Transformer and TCN on same window, side-by-side.")
    _default_tcn_ckpt = os.path.join(_HERE, "checkpoints", "tcn_model.pt")
    _default_tf_ckpt  = os.path.join(_TRANSFORMER, cfg.checkpoint_path)
    parser.add_argument("--db",                 type=str,   default=cfg.test_db_path)
    parser.add_argument("--checkpoint",         type=str,   default=_default_tcn_ckpt)
    ...
    if not os.path.exists(args.checkpoint):
        sys.exit(f"TCN checkpoint not found: {args.checkpoint}")

    print(f"  Loading TCN model from {args.checkpoint} ...")
    tcn_model = load_tcn_model(args.checkpoint, args.device)
    ...
    if args.random and args.count > 1:
        ...
        for mmsi in selected:
            result = _process_vessel(mmsi, args, tcn_model)
    ...
    if args.all_windows:
        ...
        for wi, (enc, dec) in enumerate(windows):
            mu_pred, sigma_pred = predict_tcn(tcn_model, enc, args.device)
        ...
        print(f"\n  === TCN Aggregate over {len(windows)} windows ({total_steps} steps) ===")
    ...
    if args.compare and tf_model is not None:
        ...
        print_compare_table(args.mmsi, w_idx, dec,
                            tf_mu, tf_sigma, tcn_mu, tcn_sigma,
                            args.anomaly_threshold)
        if args.plot:
            out = args.plot_out if args.plot_out != _DEFAULT_MAP_OUT else _DEFAULT_COMPARE_OUT
            save_compare_map(enc_raw, dec, tf_mu, tcn_mu, args.mmsi, out)
    else:
        print("TCN:")
        print_window_result(dec, tcn_mu, tcn_sigma, args.anomaly_threshold)
        save_folium_map([(full_track_raw, enc_raw, dec, tcn_mu, args.mmsi)], args.plot_out)
